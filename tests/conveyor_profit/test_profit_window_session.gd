...
func _initialize() -> void:
	...
	_check(session.passing_profit == 211, "threshold is ceil of ninety percent")
	...
	_check(passing.get_efficiency_percent(211) == 90, "efficiency uses online baseline")
	var metrics: Dictionary = route_session.get_developer_metrics(210)
	...
	_check(int(metrics["omniscient_profit"]) >= 234, "developer metrics retain omniscient total")
