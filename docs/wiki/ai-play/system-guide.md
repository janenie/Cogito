...
- `ai_play/` 下的 Python 进程是 stdio MCP Server，负责暴露 `observe`、`act`、`stop` 工具、验证 DTO，并通过本机回环 WebSocket 桥与 Godot 串行交换观察和动作结果。
...
- 外部 AI 只能通过 MCP 工具接收文档规定的相机图像、可见交互文本、获准公开的玩家状态、动作结果和运行时按键绑定。
...
- Python 和 GDScript 两端的协议常量、数据包字段、动作名称、数值边界和上下文门控必须保持同步。版本 2 的桥协议使用 `action_batch`、`action_results`、`game_over`、`stop_request` 和 `stop_ack` 明确关联一个观察回合。
- 所有不可信数据都必须在两端验证。保留精确字段检查、有限数检查、观察编号关联、每批最多三个动作，以及改变上下文的动作必须位于批次末尾等规则。
- Godot 的 JSON 解析会把数值规范化为浮点；其接收边界将非布尔且数值精确等于 `2` 的 `protocol_version` 规范化为整数 `2`，并将有限安全整数 `observation_id` 规范化为整数后再发出桥信号或发送 `stop_ack`。字符串、布尔、非整数和越界 ID 必须继续被拒绝。
