...
- 普通动作回合只有在相同观察编号的 `action_results` 与不同编号的后续 observation 都到达后
  才能完成；协议允许二者乱序，任一缺失都必须继续等待。Godot 断线后重新附加时，Python 必须
  清除旧终局和缓存观察，直到新连接第一帧到达前都不得把旧截图返回给 `observe`。
...
已持久化的短 caption。每个新 RGB/深度图使用二进制 SHA-256 作为 `image_id`；当前游戏模型必须
在原本的 Responses 请求中一并返回视觉说明，不增加 API 调用。RGB caption 描述物体、相对位置、
可读文字/UI 和交互目标，深度 caption 描述障碍、路线、门口与空间结构；每条简体中文说明最多
200 字，只陈述画面事实。尚未成功说明的图片始终保留，不受历史上限裁剪；上限为 0 时，在说明
成功后只保留最新图片组。每个正式终局后仍切换干净 Codex turn。

provider 请求只记录 metadata-only 图片审计：请求序号、实际转发字节数、裁剪前图片数、实际转发
图片数、caption 数、历史上限、最新组图片数、实际转发图片的 MIME/字节数/SHA-256，以及是否有
`previous_response_id` 和 `store` 布尔值。该审计不保存 prompt、图片 Base64/URL 或响应正文。
实际图片以内容寻址二进制保存在同级私有 `provider_caption_images/`；模型 ID、`image_id`、MIME、
字节数、相对路径和 caption 写入 `provider_image_captions.jsonl`，JSONL 不含 Base64。相同 SHA-256
去重；内容日志不进入 MCP 结果、AWM 或可信轨迹。
...
Base64/URL、提示词、工具参数、响应或 key。`provider_image_captions.jsonl` 以 `0600` 保存模型生成
的 caption 和索引，`provider_caption_images/` 以 `0700` 保存权限为 `0600` 的原始图片二进制。
metadata 审计可用 SHA-256 对照这些图片或可信轨迹截图。图片注册失败时代理在请求外发前失败关闭；
caption 缺失、重复、未知、为空、过长或持久化失败时，代理在响应转发前返回 502。
...
更早的图片历史按上述上限压缩为最近原图与真实 caption。代理给每张待说明图片附加可信
`image_id` 标签和说明协议，让当前游戏模型在同一个正常响应中输出一个内部 envelope；整个响应
经缓冲、精确覆盖校验和持久化后，标签与 envelope 会在转发给 Codex 前剥离。
provider 返回的 `cogito_ai_play:<tool>` 形式只会在 `<tool>` 属于当前严格白名单时规范化为相同
namespace 调用。
代理只接受 `POST /v1/responses`，除上述 metadata 审计、caption JSONL 与内容寻址图片外，不持久化
请求、响应或 key，并纳入统一子进程 readiness、失败和退出清理。API key 仍只存在于 Codex 玩家
环境，代理环境与启动参数不含 key。
...
calls。后续 turn 会把 Codex 历史 `input` 中的短工具名和 `namespace` 重新展平，为已完成的
assistant message 和函数调用补入 Yibu 要求的 `status`，并拒绝非白名单
历史调用。响应侧逐 SSE frame 验证所有 function call，将扁平名还原为短工具名与 MCP namespace 后
再交给 Codex 路由。未知工具、重复别名、非法 JSON、破损或无终局 SSE 都失败关闭；代理不重试
上游请求。Yibu 的最终 terminal frame 可以省略 SSE 空行分隔；合法终局后也可以消费追加的
keepalive 或 `[DONE]`，但其他终局后 JSON 事件仍失败关闭。Yibu 非 2xx 状态和最多 64 KiB 的
错误正文有界转发，日志只记录安全元数据，不记录
...
- 托盘最多容纳五项食材。食材一旦选入就不可撤销；选错时只能通过 `make` 消耗全部托盘食材，
  以 `invalid_combo` 锁定当前窗口。第 6 次选材返回 `tray_full`，不扣钱且不改变五项托盘，
  调用方只能提交当前托盘，不能因选材过多断开观察连接；窗口到期时未提交的托盘也会按成本报废。
- `wait_next_window` 只在当前窗口已经制作并锁定后有效，只推进一个窗口；最后一个窗口推进后
  进入正常终局。它不允许跳过尚未制作的窗口。
- AI 的选材和制作动作与人类鼠标点击和 MAKE 按钮共用同一套托盘、配方和经济入口；两端均
  不提供撤销入口。
...
- 玩家固定从档案室门口开始，档案室门在本玩法中已打开，任务卡位于出生点附近。大厅目标
  楼梯及 CEO OFFICE 入口使用公开英文导视，CEO OFFICE 门保持打开；大厅开放楼梯使用连续
  斜坡碰撞和与可见扶手一致的侧边保护，避免原子移动因踏板缝隙或侧缘意外坠落。
- 档案室内彼此分开的三组书架共有九个作者标定槽位。每局以种子确定地选中六个槽位，保持
  每组书架两本、低中高三层各两本的均衡分布；不同种子可使用不同布局。
- 每层两本书中恰有一本任务书，但书本上方不显示悬浮身份标记。玩家靠近并逐本对准书本，
  通过画面 HUD 显示的“任务书”或“普通书”判断身份；AI 可用 `probe_interaction` 安全对准，
  再从该动作返回的截图读取 HUD 名称。该玩法只适度增加近距离交互射线长度与形状探测宽度，
...
- 非零 `round_seed` 仅供本地确定性初始化和测试。种子、已选槽位、任务书身份及其他
  ...
  中的交互 HUD 逐本观察；内部文档也不得把这些信息作为公开
  MCP 答案泄露。
...
- 每局枚举四份资料的 24 种一对一排列，并生成三条与隐藏答案一致的关系线索。三条合并后
  ...
  和三处记录分配。
- 对准任务资料触发一次拿取交互即可稳定贴近视角，无需持续按住或手动旋转；携带期间关闭
  资料碰撞，避免行走或转向时被环境拖落。
...
- 玩家用中央广场的 4 个满水壶浇完向日葵房和绣球花房各 2 块草坪；每个水壶只能浇
  1 块。兰花房的草坪不是目标。
- HUD 天气显示下雨时，玩家必须在约 10 分钟现实时间的雨期结束前按下兰花房门铃。
  浇错草坪、按错门铃、非下雨时按兰花房门铃或错过警报都会失败。
