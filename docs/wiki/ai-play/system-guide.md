...
- 托盘最多容纳四项食材，与公开菜谱的最大配方长度一致。第 5 次选材返回 `tray_full`，不扣钱、
...
- AI 的选材、撤销和制作动作与人类鼠标点击、UNDO 和 MAKE 按钮共用同一套托盘、配方和
  经济入口。
- `briefing` 白名单返回墙上同样可见的六道固定菜谱，包括食材列表、售价和净利润。
...
十分钟规则由十个连续的 60 秒窗口组成。每个窗口的传送带固定摆满 16 盘真实可选食材；固定
种子的约束生成器必须保证整批供给恰好能完成两种不同菜谱、两者净利润不同，并且不能完成
第三种菜谱。每个窗口只有一次制作机会：合法组合获得售价并扣除成本，非法组合收入为零并
扣除所选食材成本；两者都会清空托盘并锁定当前窗口。锁定后 HUD 继续显示“本时间段已完成”
及剩余时间，人工模式到 60 秒边界再进入下一窗口。

AI 模式的窗口时钟由 Godot 控制；controller 等待外部模型返回期间不推进经营倒计时，避免把
API 延迟计入策略表现。人工模式仍按真实 Godot 时间运行。整局以十个窗口各自较高候选菜净
利润之和为理论基准，实际净利润达到其向上取整的 80% 才成功。运行中不得公开单窗口最优答案、
理论最高总利润或绝对通关金额。
