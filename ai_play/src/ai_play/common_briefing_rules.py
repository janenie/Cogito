...
COMMON_CONTROL_RULES = [
    ...
    (
        "结合视角旋转和深度图测距；路径开阔、方向明确时应主动推进。只有接近可交互目标、"
        "狭窄区域、平台边缘，或 "
        "movement_feedback.blocked 为 true 时才减小步幅并修正站位，避免碰撞、卡住或从高处"
        "摔落损失生命。"
    ),
    (
        ...
        "上限为 1；"
        "单轴绝对值 1 是满强度，单轴 0.2 到 0.4 只用于已经确认需要的近距精细对位，"
        "不能作为普通导航的默认力度。"
    ),
    (
        "duration_ms 是按住移动键的毫秒数；250ms 满强度 move 约等于连续走四分之一秒，"
        "满强度 sprint 约等于连续跑四分之一秒。开阔路线优先使用满强度 150 到 250ms，"
        "远距离直线可使用 sprint；只有确认进入近距门框、狭窄通道或精细对位阶段后，才使用"
        "单轴 0.2 到 0.6、50 到 100ms，并根据 act 返回的新观察和 movement_feedback 修正。"
    ),
    (
        "如果连续两次同方向移动都未受阻且路线仍然清楚，下一次应增大力度、持续时间或在一个"
        "批次中提交最多三个同方向移动，不要继续用相同或更小的步幅逐帧确认。楼梯对齐且玩家"
        "高度持续变化时，应保持满强度 150 到 250ms 上下楼；一次移动后高度没有按预期变化时，"
        "先用 15 到 30 度转向重新对齐，不要重复许多微小直行。"
    ),
    ...
    (
        "probe_interaction 成功对准后，last_action_results 中的动作结果会通过 "
        "available_interactions 返回当前 HUD 公开的 action、binding 和 prompt；随后 observation "
        "中的同名字段会保持一致。应先用 prompt 确认具体含义，再使用 action 指定的交互槽。"
    ),
    (
