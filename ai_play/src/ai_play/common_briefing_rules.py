...
COMMON_CONTROL_RULES = [
    ...
    (
        "look 的单次参数范围是 yaw -15 到 15、pitch -15 到 15；yaw 为正向右转、"
        "为负向左转，pitch 为正向下看、为负向上看。"
    ),
    (
        "look 的角度单位是度；15 度适合扫视房间，5 到 10 度适合微调准星。"
    ),
    (
        "可以先用 look 环顾四周并灵活调整视角，每次转向后重新 observe；结合"
        "截图中的遮挡、物体大小和地面透视做深度估计，判断自己与物体的距离。"
    ),
    (
    ...
    (
        ...
        "probe_interaction 和 observe 修正站位，避免碰撞、卡住或从高处摔落损失生命。"
    ),
    (
        "move 和 sprint 的参数范围是 forward -1 到 1、right -1 到 1、duration_ms "
        "50 到 250；forward 控制前后，right 控制左右。"
    ),
    (
        "duration_ms 是按住移动键的毫秒数；250ms 满强度 move 约等于连续走四分之一秒，"
        "满强度 sprint 约等于连续跑四分之一秒。接近门、桌面或小物体时优先用 100 到 150ms。"
    ),
