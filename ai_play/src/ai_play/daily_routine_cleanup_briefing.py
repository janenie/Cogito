...
PUBLIC_BRIEFING = {
    ...
    "objective": (
        "把 4 个散落垃圾和冰箱里的过期牛奶都扔进客厅垃圾桶，确认冰箱关闭后，点击客厅垃圾桶旁边的完成按钮。"
    ),
    "success_condition": "所有目标垃圾都已扔进客厅垃圾桶、冰箱处于关闭状态，并点击完成按钮。",
    "failure_condition": "在任一完成条件尚未满足时点击完成按钮，或达到最大 act 请求数。",
    "rules": COMMON_CONTROL_RULES + [
        "先观察 HUD 的当前目标、总垃圾数、已扔数量和手上物品。",
        "可交互物体需要靠近、对准并在出现交互提示后操作。",
        "冰箱需要先打开，才能取过期牛奶；取完后最后必须把冰箱关上。",
        "手上一次只能拿一个物品；拿到垃圾后先送到客厅垃圾桶。",
        "只有客厅垃圾桶用于本局目标。",
        "完成按钮在客厅垃圾桶旁边；提交失败不会告诉你缺少哪一项条件。",
