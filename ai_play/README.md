...
Every model round also receives a derived `probe_interaction_harness`. Its
`aligned` state is true only while the current
`interface.available_interactions` list is non-empty; seeing an icon or a
historical `aligned` result is not sufficient. `not_aligned` means the latest
probe found no current prompt, `inconsistent` means an old aligned result no
longer has a current prompt, `interface_open` blocks probing, and
`ready_to_probe` means no current interaction is aligned. The harness lists only
the currently approved `interact` or `interact2` slots and never activates an
object by itself.
...
`model_input` contains the model name, current screenshot path, optional
reference-atlas path, structured observation, bounded memory, and the derived
`probe_interaction_harness`. It intentionally omits the system prompt and the
Chat Completions `messages` envelope. `model_output.raw_content` is the model
response text verbatim, including malformed JSON, action numbers, and any
candidate passwords generated by the model. Treat the run directory as
sensitive harness-debugging data. Credentials, authorization headers, and image
